...
const ClientTicketDetail = () => {
  ...
  const handleSubmitComment = async (e) => {
    e.preventDefault();
    if (!newComment.trim() && !(commentFiles && commentFiles.length)) return;
  ...
  // handle previews for comment files
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="space-y-4 mb-6 max-h-96 overflow-y-auto">
          {comments.map((comment) => (
            <div
              key={comment.id}
              className={`p-4 rounded-lg ${comment.user_id ? 'bg-primary-50 border-l-4 border-primary-500' : 'bg-gray-50 border-l-4 border-gray-400'}`}
            >
              <div className="flex justify-between items-start mb-2">
                <div>
                  <p className="font-semibold text-gray-800">{comment.user_name || comment.client_name || 'Клиент'}</p>
                  {comment.user_role && (
